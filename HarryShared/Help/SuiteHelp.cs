...
public static class SuiteHelp
{
    ...
    public static HelpContent Analysis(string version) => new(
        ...
        Sections: new List<HelpSection>
        {
            new("Look up a part", "Teil suchen", new List<HelpStep>
            {
                ...
                new("The overview line under the scanned code names all three origin modules the part passed through — M1X lubrastrip module, M2X trimmer module and M3X BLADE module — each with its nest, plus order, temperature, humidity, M50 nest and the part-exit time. A '-' means the PLC sent that field empty at part exit (about one part in five has no M2X/M3X module). Fixed 2026-08-28: the line now wraps instead of being clipped at the right edge, so the later fields (M3X, M50 nest, Created) are actually visible.",
                    "Die Übersichtszeile unter dem gescannten Code nennt alle drei Herkunftsmodule, die das Teil durchlaufen hat — M1X-Lubrastrip-Modul, M2X-Trimmer-Modul und M3X-KLINGEN-Modul — jeweils mit Nest, dazu Auftrag, Temperatur, Feuchte, M50-Nest und den Austrittszeitpunkt. Ein '-' heißt, dass die SPS das Feld beim Teile-Austritt leer geliefert hat (etwa jedes fünfte Teil hat kein M2X-/M3X-Modul). Korrigiert am 28.08.2026: die Zeile bricht jetzt um, statt rechts abgeschnitten zu werden — die hinteren Felder (M3X, M50-Nest, Created) sind dadurch überhaupt erst sichtbar."),
    ...
    public static HelpContent Pareto(string version) => new(
        ...
        DescriptionEn:
            ...
            "station's cameras (KF1/KF4) are merged into one bar and _S1.._S5 sensors into one family; " +
        ...
        DescriptionDe:
            ...
            "einer Station (KF1/KF4) zu einem Balken und _S1..S5-Sensoren zu einer Familie zusammengefasst; " +
        ...
        Sections: new List<HelpSection>
        {
            ...
            new("Read the view", "Ansicht lesen", new List<HelpStep>
            {
                ...
                new("A bar is a stack with one segment per camera (KF1/KF4, distinguished by shade) and one entry per sensor family — the hover tooltip lists the exact per-camera and per-sensor split so a skew (e.g. a feature only failing on KF4) stays visible.",
                    "Ein Balken ist ein Stack mit einem Segment je Kamera (KF1/KF4, per Helligkeit unterschieden) und je Sensor-Familie — der Tooltip beim Überfahren zeigt die genaue Aufteilung je Kamera und je Sensor, sodass eine Schieflage (z. B. ein Merkmal, das nur auf KF4 fehlschlägt) sichtbar bleibt."),
            ...
            new("Origin (click a bar)", "Herkunft (Balken anklicken)", new List<HelpStep>
            {
                ...
                new("Concentration on one module/nest → check the mechanics/process there; an even spread → more likely material or camera evaluation. Origin is only known after part exit (dmcserial), so parts that have not exited yet are missing from the matrix.",
                    "Konzentration auf ein Modul/Nest → Mechanik/Prozess dort prüfen; Gleichverteilung → eher Material oder Kameraauswertung. Die Herkunft ist erst nach dem Teile-Austritt bekannt (dmcserial), noch nicht ausgetretene Teile fehlen daher in der Matrix."),
                new("'Group by' (top right of the panel, new 2026-08-28) switches the dimension: M1X lubrastrip module, M2X trimmer module or M3X BLADE module. It is pre-selected from the feature's module_ref, but you can switch freely — a part passes through one module of each family, so every breakdown is available for every feature. This is how you see which blade module e.g. the anode cosmetic rejects come from: module_ref cannot express that (it is M2x on ST040 and NoRef on ST130). When the shown dimension is not the one module_ref points at, an orange note says so — the grouping is then your choice, not a fact derived from the template.",
                    "'Group by' (rechts oben im Fenster, neu 28.08.2026) schaltet die Dimension um: M1X-Lubrastrip-Modul, M2X-Trimmer-Modul oder M3X-KLINGEN-Modul. Vorbelegt wird sie aus dem module_ref des Merkmals, umschalten kannst du aber frei — ein Teil durchläuft je ein Modul jeder Familie, also ist jede Aufschlüsselung für jedes Merkmal verfügbar. So sieht man, aus welchem Klingenmodul z. B. die Anode-Cosmetic-Ausschüsse kommen: über module_ref geht das nicht (der steht auf ST040 auf M2x und auf ST130 auf NoRef). Zeigt die Matrix eine andere Dimension als der module_ref, weist ein oranger Hinweis darauf hin — die Gruppierung ist dann deine Wahl, keine aus dem Template abgeleitete Tatsache."),
            }),
            new("Filters, refresh, export", "Filter, Aktualisierung, Export", new List<HelpStep>
            {
                ...
                new("'Split by camera' turns off the station merge (shows KF1/KF4 as their own bars); 'Split sensors' turns off the _S1.._S5 family grouping. Both default to merged. 'Reset View' clears the bar-click filter + selection and restores the default view; clicking the already-active module bar also clears the filter.",
                    "'Split by camera' hebt die Stations-Zusammenfassung auf (zeigt KF1/KF4 als eigene Balken); 'Split sensors' hebt die _S1..S5-Familien-Gruppierung auf. Beide standardmäßig zusammengefasst. 'Reset View' setzt Balken-Filter + Auswahl zurück auf die Standardansicht; ein erneuter Klick auf den aktiven Modul-Balken hebt den Filter ebenfalls auf."),
    ...
    public static HelpContent Server(string version) => new(
        ...
        Sections: new List<HelpSection>
        {
            ...
            new("Part exit & images", "Teile-Austritt & Bilder", new List<HelpStep>
            {
                ...
                new("Production CSV column layout (changed 2026-07-28): the two strands and the two M50 ST110 control windows now SHARE their measurement columns, because they are mutually exclusive per part and their variable names are identical. Header row 1 shows the merge group — 'M1x_<Station>_<KF>' for M10/M11, 'M2x_…' for M20/M21, 'M50_ST110' for EVERY ST110 window (KF1/KF4); every other controller is unchanged. That takes the file from 722 to 431 columns WITHOUT losing a value (the removed columns were always empty). The new meta column 'M50St110Kf' (the KF digit, 1/4, empty without an ST110 measurement) sits behind M50Nest and records which control window supplied the values. Files written before the change keep the old layout — the header is fixed when a file is created, so no file mixes both. Rename 2026-08-28: the second ST110 control window is now 'M50_ST110_KF4' (was KF3); the retired KF3 camera was deactivated in the DB on the same day, so it no longer appears in pick lists. Newly created files have 439 columns. The merge group is derived from the 'M50_ST110_' PREFIX since then, not from a fixed KF list — otherwise the renamed window gets its own 216-column block again (which briefly made the live CSV 655 instead of 431 columns wide).",
                    "Spaltenlayout der Produktions-CSV (geändert am 28.07.2026): die beiden Stränge und die beiden Kontrollfenster von M50 ST110 TEILEN sich jetzt ihre Messspalten, weil sie sich je Teil ausschließen und ihre Variablennamen identisch sind. Kopfzeile 1 zeigt die Merge-Gruppe — 'M1x_<Station>_<KF>' für M10/M11, 'M2x_…' für M20/M21, 'M50_ST110' für JEDES ST110-Fenster (KF1/KF4); alle anderen Controller bleiben unverändert. Damit geht die Datei von 722 auf 431 Spalten, OHNE einen Wert zu verlieren (die entfallenen Spalten waren immer leer). Die neue Metaspalte 'M50St110Kf' (die KF-Ziffer, 1/4, leer ohne ST110-Messung) steht hinter M50Nest und hält fest, welches Kontrollfenster die Werte geliefert hat. Dateien von vor der Änderung behalten das alte Layout — der Header wird beim Anlegen der Datei festgelegt, keine Datei mischt beide. Umbenennung 28.08.2026: das zweite ST110-Kontrollfenster heißt jetzt 'M50_ST110_KF4' (vorher KF3); die stillgelegte KF3-Kamera wurde am selben Tag in der DB deaktiviert und taucht in Auswahllisten nicht mehr auf. Neu angelegte Dateien haben 439 Spalten. Die Merge-Gruppe wird seitdem über das Präfix 'M50_ST110_' bestimmt, nicht über eine feste KF-Liste — sonst bekäme das umbenannte Fenster wieder einen eigenen 216-Spalten-Block (die Live-CSV war dadurch kurzzeitig 655 statt 431 Spalten breit)."),
