...
public static class SuiteHelp
{
    ...
    public static HelpContent Server(string version) => new(
        ...
        Sections: new List<HelpSection>
        {
            ...
            {
                new("On each finished part (PLC channel 2, ST160) the server writes the dmcserial row and the production CSV row. An OK part additionally has its low-res individual images handled and — if [Collage] Collage_Generate is on — a collage built. NG parts get no collage; their low-res images are kept and removed later together with the full-res NG image.",
                    "Bei jedem fertigen Teil (SPS-Kanal 2, ST160) schreibt der Server die dmcserial-Zeile und die Produktions-CSV-Zeile. Ein GUT-Teil bekommt zusätzlich seine LowRes-Einzelbilder verarbeitet und — wenn [Collage] Collage_Generate an ist — eine Collage. NG-Teile bekommen keine Collage; ihre LowRes-Bilder bleiben und werden später zusammen mit dem HighRes-NG-Bild gelöscht."),
                new("OK-part images are then removed from the low-res folder: with [NAS] DeletePictures=true they are deleted; with DeletePictures=false they are copied to [NAS] BackupFolder\\YYYY\\MM\\DD (size-verified) and then deleted. The backup tree is aged out by [NAS] BackupRetentionDays (it has no NAS sorter, so without this it would grow forever). There is no separate 'delete after collage' switch — DeletePictures + BackupFolder control everything.",
                    "Die Bilder eines GUT-Teils werden danach aus dem LowRes-Ordner entfernt: mit [NAS] DeletePictures=true gelöscht; mit DeletePictures=false nach [NAS] BackupFolder\\JJJJ\\MM\\TT kopiert (größengeprüft) und dann gelöscht. Der Backup-Baum wird über [NAS] BackupRetentionDays nach Alter bereinigt (er hat keinen NAS-Sortierer, liefe sonst voll). Es gibt keinen separaten 'nach Collage löschen'-Schalter — DeletePictures + BackupFolder steuern alles."),
                new("DE (rejected trimmer sub-assembly): a trimmer that is scrapped before it enters a frame never gets a normal part exit — it is not a finished part. ST160 sends a DE telegram carrying the trimmer serial, and the server deletes that trimmer's images (searched by the 13-char trimmer serial across the low-res, NG and diagnostic folders, incl. the NAS-sorted YYYY\\MM\\DD day-folders). It writes NOTHING to dmcserial and NOTHING to the production CSV; the trimmer's measurements_serial_trimmer rows stay and — with the log line 'DE: n trimmer image(s) … deleted' (WARNING when none are found) — are the record of the removal.",
                    "DE (ausgeschleuste Trimmer-Baugruppe): ein Trimmer, der vor dem Einsetzen in den Rahmen verworfen wird, bekommt nie einen normalen Teile-Austritt — er ist kein fertiges Teil. ST160 sendet ein DE-Telegramm mit der Trimmer-Seriennummer, und der Server löscht die Bilder dieses Trimmers (gesucht über die 13-stellige Trimmer-Serie in den LowRes-, NG- und Diagnose-Ordnern, inkl. der vom NAS einsortierten JJJJ\\MM\\TT-Tagesordner). Es wird NICHTS in dmcserial und NICHTS in die Produktions-CSV geschrieben; die measurements_serial_trimmer-Zeilen des Trimmers bleiben und sind — zusammen mit der Log-Zeile 'DE: n trimmer image(s) … deleted' (WARNING, wenn nichts gefunden wird) — der Nachweis der Ausschleusung."),
            ...
            new("Notes", "Hinweise", new List<HelpStep>
            {
                ...
                new("Retention is central: the [Retention] section sets one age in DAYS per target (0 = never) — images (NG/Diagnostic/GoldenSample/Collage/Backup + \\Input leftovers), MSA reports, CSV exports and the database. One service runs at startup and every 24 h; production DB uses partition drop + bounded batch delete (no long locks), master data is never touched, and Database_MSA / Reports_MSA default to 0 (never) because they are QS data. Every target logs what it did; legacy retention keys still work as a deprecated fallback.",
                    "Retention ist zentral: die Sektion [Retention] legt je Ziel EIN Alter in TAGEN fest (0 = nie) — Bilder (NG/Diagnose/GoldenSample/Collage/Backup + \\Input-Überbleibsel), MSA-Reports, CSV-Exporte und die Datenbank. Ein Dienst läuft beim Start und alle 24 h; die Produktions-DB nutzt Partition-Drop + begrenztes Batch-Delete (keine langen Locks), Stammdaten werden nie angefasst, und Database_MSA / Reports_MSA stehen per Default auf 0 (nie), weil es QS-Daten sind. Jedes Ziel protokolliert, was es getan hat; alte Retention-Keys wirken weiter als veralteter Fallback."),
