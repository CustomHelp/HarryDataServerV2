    ...
    public static HelpContent Server(string version) => new(
        ...
        Sections: new List<HelpSection>
        {
            ...
            new("Part exit & images", "Teile-Austritt & Bilder", new List<HelpStep>
            {
                ...
                new("DE (scrapped part): a part discarded at ST160 is not a finished part, so the server writes NOTHING to dmcserial and NOTHING to the production CSV — it only purges the part's images. DE carries the full frame SZID (assembled part), or only the trimmer serial (loose rejected trimmer), so images are deleted by BOTH the frame SZID (19) and the trimmer serial (13) when present, across the low-res, NG and diagnostic folders (incl. the NAS-sorted YYYY\\MM\\DD day-folders). The measurement rows stay; the log line 'DE: n image(s) deleted for <serials>' (WARNING when none are found) is the record.",
                    "DE (ausgeschleustes Teil): ein an ST160 verworfenes Teil ist kein fertiges Teil, daher schreibt der Server NICHTS in dmcserial und NICHTS in die Produktions-CSV — er löscht nur die Bilder des Teils. DE trägt die volle Rahmen-SZID (montiertes Teil) oder nur die Trimmer-Serie (loser Ausschuss-Trimmer); gelöscht wird deshalb über BEIDE — Rahmen-SZID (19) UND Trimmer-Serie (13), sofern vorhanden — in den LowRes-, NG- und Diagnose-Ordnern (inkl. der vom NAS einsortierten JJJJ\\MM\\TT-Tagesordner). Die Messzeilen bleiben; die Log-Zeile 'DE: n image(s) deleted for <serials>' (WARNING, wenn nichts gefunden) ist der Nachweis."),
