    ...
    public static HelpContent Server(string version) => new(
        ...
        Sections: new List<HelpSection>
        {
            ...
            new("Part exit & images", "Teile-Austritt & Bilder", new List<HelpStep>
            {
                ...
                new("DE (rejected trimmer sub-assembly): a trimmer that is scrapped before it enters a frame never gets a normal part exit. ST160 then sends a DE telegram carrying the trimmer serial, and the server deletes that trimmer's images — searched by the 13-char trimmer serial across the low-res, NG and diagnostic folders (including the NAS-sorted YYYY\\MM\\DD day-folders). The DB rows are kept for traceability. The log shows 'DE: n trimmer image(s) … deleted', or a WARNING when none are found.",
                    "DE (ausgeschleuste Trimmer-Baugruppe): ein Trimmer, der vor dem Einsetzen in den Rahmen verworfen wird, bekommt nie einen normalen Teile-Austritt. ST160 sendet dann ein DE-Telegramm mit der Trimmer-Seriennummer, und der Server löscht die Bilder dieses Trimmers — gesucht über die 13-stellige Trimmer-Serie in den LowRes-, NG- und Diagnose-Ordnern (inklusive der vom NAS einsortierten JJJJ\\MM\\TT-Tagesordner). Die DB-Zeilen bleiben zur Nachverfolgung erhalten. Das Log zeigt 'DE: n trimmer image(s) … deleted' bzw. eine WARNING, wenn nichts gefunden wird."),
