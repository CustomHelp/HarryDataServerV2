...
namespace HarryDataServer.Theming
{
    public enum AppTheme { Dark, Light }

    /// <summary>
    /// Runtime light/dark theme switch for the main server dashboard. On each switch it REPLACES
    /// the palette <see cref="SolidColorBrush"/> resources with fresh brushes — the originals are
    /// frozen once consumed by sealed implicit-style setters, so they cannot be mutated in place.
    /// Every consumer references the palette via <c>DynamicResource</c>, so swapping the resource
    /// entries updates the UI live without reloading the window. The choice is persisted to
    /// <c>%LOCALAPPDATA%\HarrySuite\theme.txt</c>, shared with the companion tools.
    /// </summary>
